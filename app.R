...
backtesting=function(snum,startDT,endDT,in_strategy,out_strategy){
  ...
  #
  colnames(test)=c("open","high","low","close","volume","adjuted","MA5","MA10","MA20","longinsite","longoutsite","ROI")
...
ui <- fluidPage(
  ...
  navbarPage(
      ...
      sidebarLayout(
        ...
        mainPanel(
          tabsetPanel(
            ...
            tabPanel("台/美股票大盤",
                     h5("台股大盤產業中以半導體占比最多，進而延伸至美股中屬於半導體產業指數，就是費城半導體指數(SOX)，因此兩個的走勢相近。詳細資訊可參考", style = "color:white;"),
                     p(tags$li(tags$b(tags$a(href="https://www.herishare.com/taiex-trend-relationship/", "", class="externallink")),
                               "以下網址。")),
                     #選擇大盤指標
    ...
    tabPanel(
      ...
      mainPanel(
        br(),
        h5("回測的用意是擬定一組策略，然後帶回到以前的日子中，看此策略的報酬是否不錯。
        所以可以擬訂很多組策略來自行觀察哪組最佳。而我們這邊的策略是，當五日線大於十日線、十日線大於二十日線時作買進，
           當五日線一跌破二十日線時作賣出。可以先用我們設定的回測去研究此方法對哪支股票的報酬率最好。", style = "color:white;"),
        ...
        br(),
        div(style="width:1000px;",box(
          title = "", status = "primary", solidHeader = TRUE,width =NULL,height = NULL,
          withLoader( verbatimTextOutput("cr_table"), type="html", loader="pacman")
        ))
      )
    ),
    #第五頁
