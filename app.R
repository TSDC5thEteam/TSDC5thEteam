...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    navbarMenu(
      ...
      tabPanel(
        messageItem(
          textInput("copytext", "複製連結", "https://tsdc5theteam.shinyapps.io/JiuCai_Stock/?_ga=2.254567104.932936494.1653143650-1385028062.1651747187"),
          uiOutput("clip"),
          icon = icon("link")
        )),
      tabPanel(
  ...
  output$backtesting <- downloadHandler(
    ...
    content = function(file) {
      write.csv(backtesting(react_stocknum_str(),react_fun$firstdate,react_fun$todate,
                ...
                row.names=TRUE, fileEncoding = "Big5"
